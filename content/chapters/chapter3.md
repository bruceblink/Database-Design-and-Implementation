...
奇偶校验具有以下有趣且重要的特性：**任何位的值都可以从其他位的值中确定，只要您也知道奇偶校验。** 例如，假设 S = {1, 0, 1}。S 的奇偶校验为 0，因为它有偶数个 1。假设您丢失了第一个位的值。由于奇偶校验为 0，集合 {未知, 0, 1} 必须有偶数个 1；因此，您可以推断缺失的位必须是 1。对其他每个位（包括奇偶校验位）也可以进行类似的推断。
